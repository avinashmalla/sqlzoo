...
-- ON (actorid = id AND movieid = (SELECT id FROM movie WHERE title = 'Alien'))
...
/* using aliases when accessing the same table in different places

SELECT title, name
FROM movie
JOIN casting x ON movie.id = x.movieid
JOIN actor ON x.actorid = actor.id
WHERE movieid IN (
  SELECT y.movieid FROM casting y
  JOIN actor ON y.actorid = actor.id
  WHERE name='Julie Andrews')
AND ord = 1 */
...
FROM casting
  JOIN actor ON (actorid = actor.id
    AND (SELECT COUNT(ord) FROM casting WHERE actorid = actor.id AND ord = 1) >= 15)
GROUP BY name
...
WHERE movieid in  (SELECT movieid FROM casting JOIN actor ON (actorid = actor.idAND name = 'Art Garfunkel'))
